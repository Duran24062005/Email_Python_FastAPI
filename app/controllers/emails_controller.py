...
class EmailController:
    ...
    async def send_email(self, email_data: EmailCreate, user_id: int) -> EmailResponse:
        ...
        """
        try:
            result = await self.email_service.send_email(email_data, user_id)
    ...
    async def get_emails(self, page: int = 1, page_size: int = 10, user_id: int = None) -> EmailList:
        """
        ...
        return await self.email_service.get_all_emails(page, page_size, user_id)
    
    async def get_email(self, email_id: int, user_id: int) -> EmailResponse:
        """
    ...
    async def update_email(self, email_id: int, email_data: EmailUpdate, user_id: int) -> EmailResponse:
        """
        Actualiza un email
        ...
        Raises:
            HTTPException: Si el email no existe
        """
        email = await self.email_service.update_email(email_id, email_data, user_id)
    ...
    async def delete_email(self, email_id: int, user_id: int) -> dict:
        """
        Elimina un email
        ...
        Raises:
            HTTPException: Si el email no existe
        """
        success = await self.email_service.delete_email(email_id, user_id)
